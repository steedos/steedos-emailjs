...
(function (root, factory) {
...
})(undefined, function (TCPSocket, imapHandler, mimecodec, Compression) {
    'use strict';
    ...
    Imap.prototype.connect = function (Socket) {
        ...
        return new Promise(function (resolve, reject) {
            _this.socket = (Socket || TCPSocket).open(_this.host, _this.port, {
    ...
    Imap.prototype.logout = function () {
        ...
        return new Promise(function (resolve, reject) {
            _this3.socket.onclose = _this3.socket.onerror = function () {
                _this3.close().then(resolve)['catch'](reject);
            };
    ...
    Imap.prototype.enqueueCommand = function (request, acceptUntagged, options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            Object.keys(options || {}).forEach(function (key) {
                return data[key] = options[key];
            });

            acceptUntagged.forEach(function (command) {
                return data.payload[command] = [];
            });
    ...
    Imap.prototype._iterateIncomingBuffer = regeneratorRuntime.mark(function callee$1$0() {
        var match;
        return regeneratorRuntime.wrap(function callee$1$0$(context$2$0) {
            while (1) switch (context$2$0.prev = context$2$0.next) {
                case 0:
                    if (!(match = this._incomingBuffer.match(COMMAND_REGEX))) {
                        context$2$0.next = 20;
                        break;
                    }
    ...
    Imap.prototype._sendRequest = function () {
        ...
        if (typeof command.precheck === 'function') {
            ...
            precheck(context).then(function () {
                ...
                var cmd,
                    index = _this7._clientQueue.indexOf(context);
                if (index >= 0) {
                    cmd = _this7._clientQueue.splice(index, 1)[0];
                }
                if (cmd && cmd.callback) {
                    cmd.callback(err);
                    _this7._canSend = true;
                    _this7._parseIncomingCommands(_this7._iterateIncomingBuffer()); // Consume the rest of the incoming buffer
                    _this7._sendRequest(); // continue sending
                }
            });
            return;
        }
    ...
    Imap.prototype._enterIdle = function () {
        ...
        this._idleTimer = setTimeout(function () {
            return _this8.onidle && _this8.onidle();
    ...
    Imap.prototype.enableCompression = function () {
        ...
        if (typeof window !== 'undefined' && window.Worker && typeof this._workerPath === 'string') {
            ...
            this._compressionWorker.onmessage = function (e) {
                ...
                switch (message) {
                    case MESSAGE_INFLATED_DATA_READY:
                        _this9._socketOnData({
                            data: buffer
                        });
                        break;

                    case MESSAGE_DEFLATED_DATA_READY:
                        _this9.waitDrain = _this9.socket.send(buffer);
                        break;
            ...
            this._compressionWorker.onerror = function (e) {
                _this9._onError(new Error('Error handling compression web worker: Line ' + e.lineno + ' in ' + e.filename + ': ' + e.message));
            };
        ...
        this.socket.ondata = function (evt) {
            ...
            // inflate
            if (_this9._compressionWorker) {
                _this9._compressionWorker.postMessage(_this9._createMessage(MESSAGE_INFLATE, evt.data), [evt.data]);
            } else {
                _this9._compression.inflate(evt.data);
            }
        };
    };
...
//# sourceMappingURL=emailjs-imap-client-imap.js.map
